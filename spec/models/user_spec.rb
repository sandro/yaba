...
describe User do
  it 'should have a display name' do
    u = Factory.build :user, :first_name => "Joe", :last_name => "Bob"
    u.display_name.should == "Joe Bob"
  end

  it 'should send an email after creating a user' do
    deliveries = ActionMailer::Base.deliveries = []
    lambda do
      Factory :user
    end.should change(deliveries, :size).from(0).to(1)
  end

  describe 'identity urls' do
    it 'should normalize the identity url' do
      u = Factory.create :user, :identity_url => "example.com"
      u.identity_url.should == "http://example.com/"
    end

    it 'should not attempt normalizing the identity url if it did not change' do
      u = Factory.create :user, :identity_url => "example.com"

      u.should_not_receive :normalize_identity_url
      u.email = "meh@dodgit.com"
      u.save
    end
  end

  describe 'states' do
    it 'should create a user account which only has an identity_url' do
      u = Factory.build :user_with_identity_url
      u.register_with_openid!
      u.reload.should_not be_new_record
    end

    it 'should become active after verifying email' do
      u = Factory :user
      u.register_with_email!
      u.should_receive(:email_registered)
      u.activate!
      u.should be_active
    end
  end
...
describe User, "Restful Authentication" do
  fixtures :users
  ...
  describe 'allows legitimate first names:' do
    ['(7\'4", 520 lb.) -- has a posse', 
     '', '1234567890_234567890_234567890_234567890',
    ].each do |name_str|
      it "'#{name_str}'" do
        lambda do
  ...
  it 'does not rehash password' do
    ...
    users(:quentin).update_attribute(:email, 'quentin2@example.com')
...
protected
  def create_user(options = {})
    email = options.delete(:email) || 'quire@example.com'
    record = User.new({ :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.email = email
    record.save
    record
  end
