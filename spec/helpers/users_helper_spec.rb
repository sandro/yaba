...
describe UsersHelper do
  before do
    @user = stub_model(User, :first_name => "U. Surname", :email => 'user_name')
  end
  ...
  describe "link_to_user" do
    ...
    it "should use the name as link text with :content_method => :first_name" do
      link_to_user(@user, :content_method => :first_name).should have_tag("a", 'U. Surname')
    end
    it "should use the login as title with no :title_method specified" do
      link_to_user(@user).should have_tag("a[title='user_name']")
    end
    it "should use the name as link title with :content_method => :first_name" do
      link_to_user(@user, :title_method => :first_name).should have_tag("a[title='U. Surname']")
    end
  ...
  describe "link_to_current_user, When logged in" do
    ...
    it "should use the name as link text with :content_method => :first_name" do
      link_to_current_user(:content_method => :first_name).should have_tag("a", 'U. Surname')
    end
    it "should use the email as title with no :title_method specified" do
      link_to_current_user().should have_tag("a[title='user_name']")
    end
    it "should use the name as link title with :content_method => :first_name" do
      link_to_current_user(:title_method => :first_name).should have_tag("a[title='U. Surname']")
    end
